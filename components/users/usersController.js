...
module.exports.forgotpasspost = async (req, res, next) => {
    ...
    const active = await UserService.isActive(res, req.body.username);
    console.log(active + "dfsdf");

    if (active == false) {
        error = "email Chưa được xác thực";
        console.log("chưa xác thực email");
        res.render('forgetpw', { error });
        return false;

    }
    const result = await UserService.checkcode(res, req.body.code, req.body.username);
    if (result) {
